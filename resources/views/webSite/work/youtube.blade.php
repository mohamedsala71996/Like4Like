...
<div class="container">
    <div class="my-5 row">
        ...
        @else
        <form id="youtube_status" action="{{route('work.changeStatus')}}" method="post">
        @endif        
                @csrf
                <div class="w-100 form p-3 px-4 shadow mx-auto">
                    ...
                    <div class="my-3">
                        ...
                        @if($link->status == 0)
                        <a href="{{$link->link}}" target="_blank" class="btn btn-primary" id="youtub_link">اضغط على الرابط</a>
                        @else
                        <a href="{{$link->link}}" target="_blank" class="btn btn-primary" style="display:none;">اضغط على الرابط</a>
                        @endif
...
@push('scripts')
<script>
    document.getElementById("youtub_link").addEventListener("click", function() {
  document.getElementById("youtube_status").submit();
});
</script>
@endpush